...
- **Status:** VERIFIED
- **Code:** `apps/sandbox/src/hooks/useKeyboardShortcuts.ts:622` (Cmd chain: time-selection-overlapped clips promote+move `:656`–`:720`; fallback to currently selected clips `:729`–`:744`, with `return` at `:744` so no playhead nudge while Cmd is held); selection math `apps/sandbox/src/hooks/handlers/playheadSelectionHandlers.ts:15`
- **Profile-gated:** no (always active).
...
- **Behaviour:** The timeline ruler wrapper is `role="region"` aria-label "Timeline ruler", `tabIndex = useTabOrder('timeline-ruler')` (=99). When focused its own `onKeyDown` handles: **Escape** → blur the ruler (`currentTarget.blur()`); **Shift+F10** → open the ruler context menu at bottom-centre; **ArrowUp/ArrowDown** → swallowed no-op (`preventDefault` + `stopPropagation`, focus stays put — deliberately consumed so the global "single-item region → jump into track list" handler can't steal focus); **ArrowLeft/ArrowRight** → CONSUMED (`preventDefault` + `stopPropagation` — they never fall through to the global playhead handler; the ruler's own snap-aware nudge replaces it) and nudge the playhead (`SET_PLAYHEAD_POSITION`): with snap ON each press lands on the next/previous grid division and **Shift has no effect**; with snap OFF the step is 0.1s and Shift accelerates to 1s (the `e.shiftKey ? 1 : 0.1` step exists only in the snap-off branch). Mouse: right-click / context-menu opens the same menu; click-to-play (when enabled); mouse-move tracks the cursor time.
- **Status:** VERIFIED
- **Code:** wrapper `apps/sandbox/src/components/EditorLayout.tsx:524` (role/tabIndex/handlers); handler bundle `apps/sandbox/src/hooks/useTimelineRulerInteractions.ts:59` (Escape `:60`, Shift+F10 `:63`, Up/Down swallow `:77`, Left/Right consume+nudge `:86`–`:88` preventDefault/stopPropagation, snap branch `:91`–`:111`, snap-off Shift step `:113`, context menu `:169`)
- **Profile-gated:** flat → 0 (still focusable, keydown unchanged).

### Add-track group (tab stop 98) — keyboard behaviour
- **Behaviour:** The "Add new" control sits in a `role="group"` aria-label **"Add track"** (`.track-control-side-panel__add-group`) wrapping a single Button whose `tabIndex = useTabOrder('add-track')` (=98). The group's class is what the global ArrowUp/Down track-focus handler keys on to **no-op arrow keys pressed within it** (so arrows don't jump into the track list). **Enter opens the AddTrackFlyout; Space does NOT.** The button is a native `<button>`, so Enter fires its `click` (opening the flyout) on keydown, but Space's native activation happens on keyup after the keydown's default — and the global transport handler (`useKeyboardShortcuts.ts:242`, a `document` keydown listener) calls `e.preventDefault()` on Space for **any** non-text-field target, which cancels the button's pending Space activation. Net: pressing Space with the button focused **plays/pauses the transport and does not open the flyout** — a deliberate deviation from the native-button convention where Space activates. The flyout opens with `autoFocus` set when the open was keyboard-triggered (Enter). Inside the flyout: auto-focus the first option on keyboard open; **ArrowLeft/Right/Up/Down** move between options with **wrap** (updating roving tabindex); **Enter** selects the focused option (clicks it); **Space** does NOT select — the flyout's own handler `preventDefault`s it (`:161`, no select/scroll) AND the same global transport handler fires (both are `document` keydown listeners; the global one is registered at app-mount so it runs first, and neither uses `stopImmediatePropagation`), so Space inside the flyout also plays/pauses; **Tab** closes the flyout WITHOUT `preventDefault`, so tabbing genuinely continues to the next element (deliberate contrast with `ContextMenu`, whose Tab is prevented); **Escape** closes it and restores focus to the trigger button.
- **Status:** VERIFIED (spot-checked 2026-07-13 — live in sandbox editor, add-track button focused: synthetic bubbling Space keydown → `defaultPrevented === true` (global transport handler ran, suppresses native Space activation); Enter keydown → `defaultPrevented === false` (native button activates → flyout opens). The Space-plays-inside-flyout leg is code-derived from the same global handler + two-listener registration order, not separately live-driven, since the portal flyout could not be held open under synthetic/CDP clicks.)
- **Code:** group + button `packages/components/src/TrackControlSidePanel/TrackControlSidePanel.tsx:294` (role/aria-label), `:301`–`:326` (Button, `onClick` opens flyout, `tabIndex={addButtonTabIndex}`), `:184` (`useTabOrder('add-track')`); Button is a native `<button>` `packages/components/src/Button/Button.tsx:85`; global Space interceptor `apps/sandbox/src/hooks/useKeyboardShortcuts.ts:235`–`:245` (Space → `preventDefault` + `handleSpacebar` for any non-text target) registered at `:916`; flyout `packages/components/src/AddTrackFlyout/AddTrackFlyout.tsx:100` (Escape+restore), `:117` (auto-focus first), `:139` (arrow wrap), `:155` (Enter select), `:161` (Space prevent — no `stopImmediatePropagation`), `:165` (Tab close), listener registered `:171`
...
### Preferences modal — F6 / Shift+F6 region cycling (document-level, any profile)
- **Behaviour:** While the Preferences modal is open, it registers its own **document-level** keydown handler for **F6**: plain **F6** → `preventDefault` and focus the **first focusable control in the content pane** (query inside `contentRef`: `button/[href]/input/select/textarea`, each `:not([tabindex="-1"])`); **Shift+F6** → focus the **first button in the left-hand section list** (`.dialog-sidenav` → first `<button>`). The handler tracks a `focusedRegion` state ('sidebar' | 'content') but the keys are absolute jumps, not a cycle position. The listener is added only while `isOpen` and removed on close. There is **no profile check** — this runs in both profiles, unlike the app-global F6 block navigation which is wired only in flat navigation (`useKeyboardShortcuts.ts:355`). In the default AU4 profile the modal's handler is therefore the only F6 handler while the modal is open. (Caveat, recorded honestly: in the FLAT profile both document listeners fire — the global F6 handler does not check `e.defaultPrevented` (`useKeyboardShortcuts.ts:355`–`:358`) and both call `.focus()` directly, so the final focus target depends on listener registration order. Untested live; irrelevant to the default profile.)
- **Status:** VERIFIED (code)
- **Code:** `packages/components/src/PreferencesModal/PreferencesModal.tsx:67`–`:102` (handler; Shift+F6 sidebar branch `:72`–`:79`, F6 content branch `:80`–`:91`, listener gated on `isOpen` `:95`)
- **Profile-gated:** no. (Undocumented in the four docs.)
...
- **Behaviour, code-verified:**
  ...
  - **Tab / Shift+Tab** — the handler calls `preventDefault` (+ `stopImmediatePropagation`) BEFORE closing, so **no Tab focus move happens**; the menu closes and focus simply drops when the menu unmounts (it does not restore to the trigger and it does not advance to the next tab stop). Deliberate contrast with `AddTrackFlyout`, whose Tab branch does NOT `preventDefault` — there, tabbing genuinely continues to the next element after the flyout closes.
...
- **Code:** open + trigger capture `packages/components/src/ContextMenu/ContextMenu.tsx:75` (store trigger), auto-focus first `:82`, Escape close + focus-restore `:142`–`:155` (restore `:150`), Tab preventDefault+close `:157`–`:163`, ArrowDown/Up wrap `:165`–`:183`, Home/End `:185`–`:193`; item activation `packages/components/src/ContextMenuItem/ContextMenuItem.tsx:237` (Enter/Space → onClick+onClose), submenu open/close `:210`–`:234`; flyout contrast `packages/components/src/AddTrackFlyout/AddTrackFlyout.tsx:165`–`:168` (Tab: onClose, no preventDefault)
