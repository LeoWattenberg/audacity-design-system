...
- **Scope editing:** Cmd+Click on a track's control panel toggles that row in/out of `timeSelection.tracks` while a scoped selection is active; `selectedTrackIndices` is untouched. Falls back to the classic track-toggle when no scoped selection is active. (Prototype wired this through `EditorLayout` → track panel click path.)
...
- Gates: guard:any, sandbox suite, apps/sandbox tsc, build. NOTE: master currently has a pre-existing tsc/build break at `Canvas.tsx:528` (chip task_a99d7768) — if unfixed when implementation starts, fix it first or alongside, since this work touches the same file.
